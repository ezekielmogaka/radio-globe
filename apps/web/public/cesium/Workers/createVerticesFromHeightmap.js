...
import { a as ar, c as sr } from "./chunk-33IJ66H2.js";
import { a as ir } from "./chunk-ITDFUUP5.js";
import { a as xe } from "./chunk-VSMOUEKV.js";
import { a as nr } from "./chunk-AZKYNZOH.js";
import "./chunk-DAX2P4DI.js";
import "./chunk-SI3KZUTO.js";
import { a as tr } from "./chunk-QM7HI6IF.js";
import "./chunk-SPC72PTY.js";
import "./chunk-HVWI7BO6.js";
import { c as rr } from "./chunk-BNHFHP3L.js";
import "./chunk-4UHVCH4T.js";
import { i as er, j as ne } from "./chunk-4NDD6KRQ.js";
import { b as Ie } from "./chunk-5ELDAYCN.js";
import "./chunk-N6BI774S.js";
import "./chunk-TI35VBH7.js";
import { a as Re } from "./chunk-6WLI3422.js";
import { b as Pe, c as pe } from "./chunk-ICY67TC6.js";
import { a as re } from "./chunk-G2EMNOST.js";
import { a as he } from "./chunk-4VFKVGYI.js";
import { a as me } from "./chunk-3WJNS2B6.js";
import { c as pr, d as Ir, e as W } from "./chunk-XCN226AA.js";
var lr = pr((Zr, Ue) => {
  /* Copyright 2015-2018 Esri. Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with the License. You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0 @preserve */ (function () {
        ...
        ((A.defaultNoDataValue = -34027999387901484e22),
          (A.decode = function (r, a) {
            ...
            return (
              ...
              a.returnFileInfo &&
                ((u.fileInfo = b(i)),
                a.computeUsedBitDepths && (u.fileInfo.bitDepths = Y(i))),
              u
            );
          }));
          ...
          n = function (r, a, t) {
            ...
            if (
              ...
              if (
                ...
                (i.mask.numBytes | i.mask.numBlocksY | i.mask.maxValue) === 0 &&
          ...
          V = {
            ...
            readHuffman: function (n, e, r) {
              ...
              var E = f[h],
                ...
                N = F,
                _;
              for (_ = 0; _ < a.numDims; _++) {
                if (
                  (t > 1 && ((N = new r(F.buffer, s * _, s)), (B = 0)),
            ...
            readTiles: function (n, e, r) {
              ...
              var f = 0,
                ...
                le = a.numDims,
                ie,
                Z = e.pixels.resultMask,
                j = e.pixels.resultPixels;
              for (c = 0; c < l; c++)
                for (v = c !== l - 1 ? o : Te, g = 0; g < m; g++)
                  for (
                    U = g !== m - 1 ? o : ke,
                      w = c * t * o + g * o,
                      d = t - U,
                    ...
                    if (N === 2) {
                      ...
                      if (
                        ...
                        for (f = 0; f < v; f++) {
                          for (h = 0; h < U; h++) j[w++] = F[y++];
                          w += d;
                        }
                      e.ptr += y * u;
                    } else if (
                      ((Q = V.getDataTypeUsed(s, I)),
                      (z = V.getOnePixel(B, p, Q, L)),
                      (p += V.getDataTypeSize(Q)),
                      N === 3)
                    )
                      if (((e.ptr += p), e.counter.constantoffset++, Z))
                        for (f = 0; f < v; f++) {
                          for (h = 0; h < U; h++) (Z[w] && (j[w] = z), w++);
                          w += d;
                        }
                      else
                        for (f = 0; f < v; f++) {
                      ...
                      ((e.ptr += p), V.decodeBits(n, e, _, z, ie), (p = 0), Z)
                    )
                      for (f = 0; f < v; f++) {
                        for (h = 0; h < U; h++) (Z[w] && (j[w] = _[p++]), w++);
                        w += d;
                      }
                    else
                      for (f = 0; f < v; f++) {
                        for (h = 0; h < U; h++) j[w++] = _[p++];
                        w += d;
                      }
                  }
            },
      ...
      ee = {
        decode: function (A, V) {
          ...
          var b = V.inputOffset || 0,
            Y = new Uint8Array(A, b, 10),
            n = String.fromCharCode.apply(null, Y),
            e,
            r;
          if (n.trim() === "CntZImage") ((e = D), (r = 1));
          else if (n.substring(0, 5) === "Lerc2") ((e = ae), (r = 2));
    ...
    typeof define == "function" && define.amd
      ...
      : typeof Ue < "u" && Ue.exports
        ? (Ue.exports = ee)
        : (this.Lerc = ee);
  })();
});
var xr = { NONE: 0, LERC: 1 },
  fr = Object.freeze(xr);
var G = {};
G.DEFAULT_STRUCTURE = Object.freeze({
  heightScale: 1,
  heightOffset: 0,
  elementsPerHeight: 1,
  stride: 1,
...
var He = new re(),
  ...
  Tr = new re(),
  kr = new re();
G.computeVertices = function (D) {
  if (!W(D) || !W(D.heightmap)) throw new me("options.heightmap is required.");
  if (!W(D.width) || !W(D.height))
    throw new me("options.width and options.height are required.");
  if (!W(D.nativeRectangle))
    throw new me("options.nativeRectangle is required.");
  if (!W(D.skirtHeight)) throw new me("options.skirtHeight is required.");
  let ae = Math.cos,
    C = Math.sin,
    ee = Math.sqrt,
    A = Math.atan,
    ...
    t = a > 0,
    i = D.isGeographic ?? !0,
    o = D.ellipsoid ?? pe.default,
    s = 1 / o.maximumRadius,
    u = ne.clone(D.nativeRectangle),
    m = ne.clone(D.rectangle),
    l,
    f,
    h,
    c;
  W(m)
    ...
    : i
      ? ((l = Y(u.west)), (f = Y(u.south)), (h = Y(u.east)), (c = Y(u.north)))
      : ((l = u.west * s),
        (f = b - 2 * A(V(-u.south * s))),
        (h = u.east * s),
        (c = b - 2 * A(V(-u.north * s))));
  let g = D.relativeToCenter,
    v = W(g);
  g = v ? g : re.ZERO;
  let U = D.includeWebMercatorT ?? !1,
    M = D.exaggeration ?? 1,
    S = D.exaggerationRelativeHeight ?? 0,
    k = M !== 1,
    w = D.structure ?? G.DEFAULT_STRUCTURE,
    d = w.heightScale ?? G.DEFAULT_STRUCTURE.heightScale,
    x = w.heightOffset ?? G.DEFAULT_STRUCTURE.heightOffset,
    T = w.elementsPerHeight ?? G.DEFAULT_STRUCTURE.elementsPerHeight,
    y = w.stride ?? G.DEFAULT_STRUCTURE.stride,
    p = w.elementMultiplier ?? G.DEFAULT_STRUCTURE.elementMultiplier,
    L = w.isBigEndian ?? G.DEFAULT_STRUCTURE.isBigEndian,
    B = ne.computeWidth(u),
    E = ne.computeHeight(u),
    O = B / (e - 1),
    F = E / (r - 1);
  i || ((B *= s), (E *= s));
  let N = o.radiiSquared,
    ...
    Q = 65536,
    z = -65536,
    le = er.eastNorthUpToFixedFrame(g, o),
    ie = Ie.inverseTransformation(le, Ur),
    Z,
    j;
  U &&
    ((Z = xe.geodeticLatitudeToMercatorAngle(f)),
    (j = 1 / (xe.geodeticLatitudeToMercatorAngle(c) - Z)));
  let se = Tr;
  ((se.x = Number.POSITIVE_INFINITY),
    (se.y = Number.POSITIVE_INFINITY),
    (se.z = Number.POSITIVE_INFINITY));
  let fe = kr;
  ...
  for (let H = Me; H < Se; ++H) {
    ...
    (J < 0 && (J = 0), J >= r && (J = r - 1));
    let q = u.north - F * J;
    i ? (q = Y(q)) : (q = b - 2 * A(V(-q * s)));
    let Ee = (q - f) / (c - f);
    Ee = he.clamp(Ee, 0, 1);
    let de = H === Me,
      we = H === Se - 1;
    a > 0 && (de ? (q += ve * E) : we && (q -= ve * E));
    let Ge = ae(q),
      Le = C(q),
      Qe = ke * Le,
      Je;
    U && (Je = (xe.geodeticLatitudeToMercatorAngle(q) - Z) * j);
    for (let ce = Ve; ce < De; ++ce) {
      let K = ce;
      (K < 0 && (K = 0), K >= e && (K = e - 1));
      let Be = J * (e * y) + K * y,
        R;
      if (T === 1) R = n[Be];
        ...
        R = 0;
        let X;
        if (L) for (X = 0; X < T; ++X) R = R * p + n[Be + X];
        else for (X = T - 1; X >= 0; --X) R = R * p + n[Be + X];
      }
      ((R = R * d + x), (z = Math.max(z, R)), (Q = Math.min(Q, R)));
      let $ = u.west + O * K;
      i ? ($ = Y($)) : ($ = $ * s);
      let Fe = ($ - l) / (h - l);
      Fe = he.clamp(Fe, 0, 1);
      ...
      ((oe.x = gr + be * R),
        (oe.y = vr + Ne * R),
        (oe.z = dr + Le * R),
        Ie.multiplyByPoint(ie, oe, He),
        re.minimumByComponent(He, se, se),
        re.maximumByComponent(He, fe, fe),
        (ye = Math.min(ye, R)),
        (ge[P] = oe),
        (Xe[P] = new Pe(Fe, Ee)),
        (Ye[P] = R),
        U && (_e[P] = Je),
        k && (ze[P] = o.geodeticSurfaceNormal(oe)));
    }
  }
  let mr = rr.fromPoints(ge),
    Ze;
  W(m) && (Ze = nr.fromRectangle(m, Q, z, o));
  let je;
  v &&
    (je = new ar(o).computeHorizonCullingPointPossiblyUnderEllipsoid(g, ge, Q));
  let hr = new tr(se, fe, g),
    Ae = new sr(g, hr, ye, z, le, !1, U, k, M, S),
    qe = new Float32Array(te * Ae.stride),
    We = 0;
  for (let H = 0; H < te; ++H)
    We = Ae.encode(qe, We, ge[H], Xe[H], Ye[H], void 0, _e[H], ze[H]);
  return {
    vertices: qe,
    maximumHeight: z,
    minimumHeight: Q,
...
var ur = Ir(lr(), 1);
function yr(D, ae) {
  if (D.encoding === fr.LERC) {
    let A;
    try {
      A = ur.default.decode(D.heightmap);
    } catch (b) {
      throw new Re(b);
    }
    if (A.statistics[0].minValue === Number.MAX_VALUE)
      throw new Re("Invalid tile data");
    ((D.heightmap = A.pixels[0]), (D.width = A.width), (D.height = A.height));
  }
  ((D.ellipsoid = pe.clone(D.ellipsoid)),
    (D.rectangle = ne.clone(D.rectangle)));
  let C = or.computeVertices(D),
    ee = C.vertices;
  return (
    ae.push(ee.buffer),
    {
