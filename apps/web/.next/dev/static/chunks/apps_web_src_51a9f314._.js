...
class StationLoadingUseCase {
    ...
    async loadStationsForView(zoomLevel) {
        try {
            ...
            console.log(`Loaded ${stations.length} stations for zoom level ${zoomLevel}:`, stations.slice(0, 3));
            // Group stations by city for clustering
            const clusteredMarkers = this.clusterStationsByCity(stations, zoomLevel);
            console.log(`Created ${clusteredMarkers.length} marker clusters`);
            this.eventBus.notify(__TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$events$2f$GlobeEventBus$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["createGlobeEvent"].stationsLoaded(clusteredMarkers.length, 'global', zoomLevel));
            return clusteredMarkers;
        } catch (error) {
            console.error('Error in loadStationsForView:', error);
            this.eventBus.notify(__TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$events$2f$GlobeEventBus$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["createGlobeEvent"].error('Failed to load stations', 'StationLoadingUseCase', error instanceof Error ? error : new Error(String(error))));
            return [];
        }
    }
    clusterStationsByCity(stations, zoomLevel) {
        const cityGroups = new Map();
        console.log('Clustering stations:', stations.length);
        // Group stations by city
        stations.forEach((station, index)=>{
            const city = station.location?.city || station.city || 'Unknown';
        ...
        console.log(`Created ${cityGroups.size} city groups`);
        // Convert to marker data
        return Array.from(cityGroups.entries()).map(([cityKey, groupStations])=>{
            const representative = groupStations[0];
            let lat = representative.location?.latitude || representative.location?.lat || 0;
            let lng = representative.location?.longitude || representative.location?.lng || 0;
            // If no coordinates, generate some based on city name hash for demo purposes
            if (lat === 0 && lng === 0) {
                const hash = this.hashString(cityKey);
                lat = hash % 180 - 90; // -90 to 90
                lng = hash * 7 % 360 - 180; // -180 to 180
            }
            ...
            return {
                ...
                city: representative.location?.city || representative.city || 'Unknown',
                country: representative.location?.country || representative.country || 'Unknown',
                isVisible: true,
                zoomLevel,
...
class StationRepository {
    ...
    async getStationsByRegion(region) {
        ...
        try {
            ...
            const stations = await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$services$2f$radioDiscoveryService$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["radioDiscoveryService"].getTopStations(200);
...
function ClusteredStationMarkers() {
    ...
    const [currentZoomLevel, setCurrentZoomLevel] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(1);
    ...
    // Update zoom level only when it actually changes
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "ClusteredStationMarkers.useEffect": ()=>{
            if (Math.abs(zoomLevel - currentZoomLevel) > 0.25) {
        ...
        "ClusteredStationMarkers.useEffect": ()=>{
            const loadMarkers = {
                "ClusteredStationMarkers.useEffect.loadMarkers": async ()=>{
                    setIsLoading(true);
                    try {
                        const newMarkers = await stationLoadingUseCase.loadStationsForView(currentZoomLevel);
...
_s(ClusteredStationMarkers, "eQIOy0HcoixvlvD9/UPTR8nRWUU=", false, function() {
    return [
...
//# sourceMappingURL=apps_web_src_51a9f314._.js.map