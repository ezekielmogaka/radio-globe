        ...
        children: [
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$react$2d$three$2f$drei$2f$core$2f$shapes$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Sphere"], {
                ...
                children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("meshPhongMaterial", {
                    ...
                    shininess: 2,
                    specular: new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$three$2f$build$2f$three$2e$core$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Color"](0x222222),
                    transparent: false
                }, void 0, false, {
...
"[project]/apps/web/src/services/interfaces.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
class ProgressiveLoadingStrategy {
    stationRepository;
    cacheService;
    constructor(stationRepository, cacheService){
        this.stationRepository = stationRepository;
        this.cacheService = cacheService;
    }
    async getStationsForZoomLevel(zoomLevel, visibleRegion) {
        const cacheKey = this.generateCacheKey(zoomLevel, visibleRegion);
        // Try cache first
        const cached = this.cacheService.get(cacheKey);
        if (cached) return cached;
        // Calculate appropriate station count
        const stationCount = this.calculateStationCount(zoomLevel);
        let stations;
        if (visibleRegion && zoomLevel > 3) {
            // High zoom - get regional stations
            stations = await this.stationRepository.getStationsByRegion(visibleRegion);
            stations = stations.slice(0, stationCount);
        } else {
            // Low zoom - get global top stations
            stations = await this.stationRepository.getTopStations(stationCount);
        }
        // Cache result
        this.cacheService.set(cacheKey, stations);
        return stations;
    }
    calculateStationCount(zoomLevel) {
        return Math.min(2000, Math.max(100, Math.floor(zoomLevel * 300)));
    }
    generateCacheKey(zoomLevel, region) {
        if (region) {
            return `stations:${zoomLevel}:${region.north}:${region.south}:${region.east}:${region.west}`;
        }
        return `stations:global:${zoomLevel}`;
    }
}
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
...
"[project]/apps/web/src/domain/usecases.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "StationClusteringUseCase",
    ()=>StationClusteringUseCase,
    "StationLoadingUseCase",
    ()=>StationLoadingUseCase,
    "StationSearchUseCase",
    ()=>StationSearchUseCase,
    "StationSelectionUseCase",
    ()=>StationSelectionUseCase
]);
class StationClusteringUseCase {
    stationRepository;
    constructor(stationRepository){
        this.stationRepository = stationRepository;
    }
    async clusterStationsByCity(stations, zoomLevel) {
        const clusterMap = new Map();
        stations.forEach((station)=>{
            const cityKey = `${station.location.city}-${station.location.country}`;
            if (!clusterMap.has(cityKey)) {
                const markerData = {
                    id: cityKey,
                    position: this.calculatePosition(station.location.lat, station.location.lng),
                    stations: [],
                ...
                clusterMap.set(cityKey, markerData);
            }
            const cluster = clusterMap.get(cityKey);
            cluster.stations.push(station);
            // Update density based on station count
            cluster.metadata.density = this.calculateDensity(cluster.stations.length);
        });
...
class StationLoadingUseCase {
    loadingStrategy;
    clusteringUseCase;
    constructor(loadingStrategy, clusteringUseCase){
        this.loadingStrategy = loadingStrategy;
        this.clusteringUseCase = clusteringUseCase;
    }
    async loadStationsForView(zoomLevel, visibleRegion) {
        try {
            // Load stations using strategy
            const stations = await this.loadingStrategy.getStationsForZoomLevel(zoomLevel, visibleRegion);
            // Cluster the stations
            const markers = await this.clusteringUseCase.clusterStationsByCity(stations, zoomLevel);
            return markers;
        } catch (error) {
...
class StationSelectionUseCase {
    ...
    notifyListeners() {
        this.listeners.forEach((listener)=>listener(this.selectedStation));
    }
}
class StationSearchUseCase {
    ...
    async searchStations(query) {
        ...
        try {
            return await this.stationRepository.searchStations(query);
        } catch (error) {
            console.error('Search failed:', error);
            return [];
        }
    }
    filterStationsByGenre(stations, genre) {
        if (genre === 'all') return stations;
        return stations.filter((station)=>station.genre?.toLowerCase().includes(genre.toLowerCase()));
    }
    sortStationsByPopularity(stations) {
        return [
            ...stations
...
"[project]/apps/web/src/services/radioDiscoveryService.ts [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {

const e = new Error("Could not parse module '[project]/apps/web/src/services/radioDiscoveryService.ts'\n\nUnexpected token `/`. Expected identifier, string literal, numeric literal or [ for the computed key");
e.code = 'MODULE_UNPARSABLE';
throw e;
}),
"[project]/apps/web/src/infrastructure/ServiceContainer.ts [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {

const e = new Error("Could not parse module '[project]/apps/web/src/infrastructure/ServiceContainer.ts'\n\nExpected ',', got ':'");
e.code = 'MODULE_UNPARSABLE';
throw e;
}),
...
function ClusteredStationMarkers() {
    ...
    const stationLoadingUseCase = (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$infrastructure$2f$ServiceContainer$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["createStationLoadingUseCase"])();
    const stationSelectionUseCase = (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$infrastructure$2f$ServiceContainer$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["createStationSelectionUseCase"])();
    const markerFactory = (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$infrastructure$2f$ServiceContainer$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["createMarkerFactory"])();
    // Local state
    ...
    const { cameraDistance, zoomLevel } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "ClusteredStationMarkers.useMemo": ()=>{
...
_s(ClusteredStationMarkers, "yCdpDpSSEMx/YmdypBQxrmL1vE0=", false, function() {
    return [
...
function Globe() {
    _s();
    const { addMarker, clearMarkers } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$stores$2f$globe$2d$store$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useGlobeStore"])();
    const [isLoading, setIsLoading] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(true);
    const [loadedStations, setLoadedStations] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(0);
    const cameraRef = (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useRef"])();
    // Progressive station loading based on zoom level
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "Globe.useEffect": ()=>{
            const loadStations = {
                "Globe.useEffect.loadStations": async ()=>{
                    try {
                        setIsLoading(true);
                        console.log("Loading radio stations progressively...");
                        // Start with a reasonable number of top stations
                        const stations = await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$services$2f$radioDiscoveryService$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["radioDiscoveryService"].getProgressiveStations(2);
                        console.log("Loaded stations:", stations.length);
                        // Clear existing markers and add new ones
                        clearMarkers();
                        stations.forEach({
                            "Globe.useEffect.loadStations": (station)=>{
                                console.log("Adding station:", station.name, station.location);
                                addMarker(station);
                            }
                        }["Globe.useEffect.loadStations"]);
                        setLoadedStations(stations.length);
                    } catch (error) {
                        console.error("Failed to load stations:", error);
                    } finally{
                        setIsLoading(false);
...
_s(Globe, "Ea9gua5xwqppJ7fr6d2C5G501X4=", false, function() {
    return [
        __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$stores$2f$globe$2d$store$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useGlobeStore"]
    ];
});
...
//# sourceMappingURL=apps_web_src_8a44bc11._.js.map