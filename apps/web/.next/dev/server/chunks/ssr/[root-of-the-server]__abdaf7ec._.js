...
"[project]/apps/web/src/services/location/wofClient.ts [app-ssr] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function fetchWofGeoJson(id) {
...
"[project]/apps/web/src/data/wof/wofIndex.json (json)", ((__turbopack_context__) => {

__turbopack_context__.v({"generatedAt":"2026-01-14T11:50:36.718Z","sources":{"countries":"unavailable","regions":"unavailable"},"countries":{"KE":{"id":85632215,"name":"Kenya","isoAlpha2":"KE","isoAlpha3":"KEN","centroid":{"lat":0.4253,"lng":37.8626},"bbox":{"minLat":-4.678047,"minLng":33.908085,"maxLat":5.506,"maxLng":41.906895},"population":47564296},"US":{"id":85633793,"name":"United States","isoAlpha2":"US","isoAlpha3":"USA","centroid":{"lat":39.833333,"lng":-98.585522},"bbox":{"minLat":18.7763,"minLng":-171.791111,"maxLat":71.357764,"maxLng":-66.96466},"population":331002651}},"regions":{"KE":[{"id":1108730523,"name":"Nairobi","countryIso":"KE","centroid":{"lat":-1.28333,"lng":36.816667},"bbox":{"minLat":-1.42263,"minLng":36.662109,"maxLat":-1.155822,"maxLng":36.951904},"population":4397073}],"US":[{"id":85688651,"name":"California","countryIso":"US","centroid":{"lat":37.271875,"lng":-119.270416},"bbox":{"minLat":32.528832,"minLng":-124.482003,"maxLat":42.009517,"maxLng":-114.131211},"population":39538223}]}});}),
"[project]/apps/web/src/services/location/wofIndex.ts [app-ssr] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$data$2f$wof$2f$wofIndex$2e$json__$28$json$29$__ = __turbopack_context__.i("[project]/apps/web/src/data/wof/wofIndex.json (json)");
;
const wofIndex = __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$data$2f$wof$2f$wofIndex$2e$json__$28$json$29$__["default"];
function getCountryIndex() {
    return wofIndex.countries;
}
function getRegionsIndex() {
...
function findCountryByName(name) {
    ...
    return entries.find((entry)=>entry.name.toLowerCase() === normalized) || entries.find((entry)=>entry.name.toLowerCase().includes(normalized)) || null;
}
function getRegionsByCountryIso(isoCode) {
    if (!isoCode) return [];
    const normalized = isoCode.trim().toUpperCase();
...
var __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$services$2f$location$2f$wofClient$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/apps/web/src/services/location/wofClient.ts [app-ssr] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$services$2f$location$2f$wofIndex$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/apps/web/src/services/location/wofIndex.ts [app-ssr] (ecmascript)");
;
;
;
;
const regionGeoJsonCache = new Map();
function isCoordinateWithinBoundingBox(bbox, lat, lng) {
    return lat >= bbox.minLat && lat <= bbox.maxLat && lng >= bbox.minLng && lng <= bbox.maxLng;
}
function buildHierarchyEntry(id, placetype, name, bbox) {
    return {
        id,
        placetype,
        name,
...
async function resolveRegionsForCoordinate(countryIso, lat, lng) {
    if (!countryIso) return null;
    const regions = (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$services$2f$location$2f$wofIndex$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["getRegionsByCountryIso"])(countryIso);
    if (!regions.length) return null;
    const candidates = regions.filter((region)=>region.bbox ? isCoordinateWithinBoundingBox(region.bbox, lat, lng) : false);
    if (candidates.length === 1) {
        return candidates[0];
    }
    for (const candidate of candidates){
        const cached = regionGeoJsonCache.get(candidate.id);
        const geojson = cached?.geojson ?? await fetchRegionGeoJson(candidate.id);
        if (!geojson) continue;
        if (geojson.type === "FeatureCollection") {
            const hit = geojson.features.some((feature)=>containsPoint(feature, lat, lng));
            if (hit) {
                return candidate;
            }
        } else if (containsPoint(geojson, lat, lng)) {
            return candidate;
        }
    }
...
function containsPoint(feature, lat, lng) {
    try {
        ...
        console.warn("Failed to evaluate GeoJSON polygon", error);
        return false;
    }
}
async function fetchRegionGeoJson(id) {
    if (regionGeoJsonCache.has(id)) {
        return regionGeoJsonCache.get(id)?.geojson ?? null;
    }
    const geojson = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$services$2f$location$2f$wofClient$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["fetchWofGeoJson"])(id);
    regionGeoJsonCache.set(id, {
        geojson: geojson ?? null
    });
    return geojson;
}
function resolveCountry(station) {
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$services$2f$location$2f$wofIndex$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["getCountryByIso"])(station.location.countryCode) || (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$services$2f$location$2f$wofIndex$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["findCountryByName"])(station.location.country) || null;
}
function createBaseWofDetail(country) {
    if (!country) {
        return {
            hierarchy: []
        };
    ...
    const hierarchy = [
        buildHierarchyEntry(country.id, "country", country.name, country.bbox ?? undefined)
    ];
    return {
        countryId: country.id,
        hierarchy,
        centroid: country.centroid ?? undefined,
        bbox: country.bbox ?? undefined
    };
}
async function enrichStationsWithWof(stations) {
    const enriched = [];
    for (const station of stations){
        if (station.location.wof?.countryId) {
            enriched.push(station);
            continue;
        }
        const country = resolveCountry(station);
        const base = createBaseWofDetail(country);
        if (country) {
            const region = await resolveRegionsForCoordinate(country.isoAlpha2 ?? null, station.location.lat, station.location.lng);
            if (region) {
                base.regionId = region.id;
                base.hierarchy.push(buildHierarchyEntry(region.id, "region", region.name, region.bbox ?? undefined));
                if (region.bbox) {
                    base.bbox = region.bbox;
                }
                if (region.centroid) {
                    base.centroid = region.centroid;
                }
            }
        }
        enriched.push({
...
;
;
const DATA_DIR = (0, __TURBOPACK__imported__module__$5b$externals$5d2f$node$3a$path__$5b$external$5d$__$28$node$3a$path$2c$__cjs$29$__["resolve"])(process.cwd(), "src/data/naturalearth");
const COUNTRIES_FILE = (0, __TURBOPACK__imported__module__$5b$externals$5d2f$node$3a$path__$5b$external$5d$__$28$node$3a$path$2c$__cjs$29$__["resolve"])(DATA_DIR, "admin0-countries.geojson");
const REGIONS_FILE = (0, __TURBOPACK__imported__module__$5b$externals$5d2f$node$3a$path__$5b$external$5d$__$28$node$3a$path$2c$__cjs$29$__["resolve"])(DATA_DIR, "admin1-regions.geojson");
let countryCache = null;
let regionCache = null;
async function loadCollection(path) {
    const contents = await (0, __TURBOPACK__imported__module__$5b$externals$5d2f$node$3a$fs$2f$promises__$5b$external$5d$__$28$node$3a$fs$2f$promises$2c$__cjs$29$__["readFile"])(path, "utf8");
...
async function loadNaturalEarthCountries() {
    if (!countryCache) {
        countryCache = await loadCollection(COUNTRIES_FILE);
    }
    return countryCache;
}
async function loadNaturalEarthRegions() {
    if (!regionCache) {
        regionCache = await loadCollection(REGIONS_FILE);
    }
    return regionCache;
}
function findCountryFeatureByIso(isoCode) {
    ...
    const normalized = isoCode.trim().toUpperCase();
    return countryCache.features.find((feature)=>feature.properties.isoA2?.toUpperCase() === normalized) || countryCache.features.find((feature)=>feature.properties.isoA3?.toUpperCase() === normalized) || null;
}
function findRegionFeature(isoA2, iso3166_2) {
    ...
    return regionCache.features.find((feature)=>{
        const props = feature.properties;
        if (normalizedIso3166 && props.iso3166_2?.toUpperCase() === normalizedIso3166) {
            return true;
        }
        if (inCountry && props.isoA2?.toUpperCase() === inCountry && props.isoA3?.toUpperCase() === normalizedIso3166) {
            return true;
        }
        return false;
    }) || null;
   ...
   */ wrap(data, source) {
    ...
    async hydrateFromServerCache(force = false) {
        ...
        try {
            ...
            const fallback = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$services$2f$location$2f$wofEnrichmentService$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["enrichStationsWithWof"])(this.getFallbackStations());
...
//# sourceMappingURL=%5Broot-of-the-server%5D__abdaf7ec._.js.map