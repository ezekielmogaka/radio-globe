...
class RadioDiscoveryService {
   ...
   */ async searchOnlineStations(query, limit = 50) {
        ...
        const stations = this.searchLocalStations(query);
   ...
   */ async getStationsByCountry(country, limit = 100) {
        ...
        const cached = this.getFromCache(cacheKey);
        const base = cached || this.filterByCountry(normalized);
   ...
   */ async getGlobalStationsWithDistribution() {
        ...
        const distributed = this.distributeStationsByCountry(this.getAllCachedStations(), 5000);
   ...
   */ async getStationsByTag(tag, limit = 50) {
        ...
        const cached = this.getFromCache(cacheKey);
        if (cached) return cached.slice(0, limit);
        const matches = this.getAllCachedStations().filter((station)=>{
            const genre = station.genre?.toLowerCase() || "";
            const tags = station.tags?.map((t)=>t.toLowerCase()) || [];
            return genre.includes(normalized) || tags.some((t)=>t.includes(normalized));
        });
        const sorted = matches.sort((a, b)=>(b.listeners || 0) - (a.listeners || 0));
   ...
   */ async getStationsByCity(city, limit = 50) {
        ...
        const cached = this.getFromCache(cacheKey);
        if (cached) return cached.slice(0, limit);
        const matches = this.getAllCachedStations().filter((station)=>station.location.city?.toLowerCase().includes(normalized));
        const sorted = matches.sort((a, b)=>(b.listeners || 0) - (a.listeners || 0));
        this.setCache(cacheKey, sorted);
        return sorted.slice(0, limit);
    }
   ...
   */ async getCountries() {
        ...
        this.getAllCachedStations().forEach((station)=>{
            const country = station.location.country || "Unknown";
   ...
   */ getFallbackStations() {
    ...
    searchLocalStations(query) {
        ...
        const results = this.getAllCachedStations().filter((station)=>{
    ...
    filterByCountry(normalizedCountry) {
        const all = this.getAllCachedStations();
...
//# sourceMappingURL=apps_web_src_5309eec3._.js.map