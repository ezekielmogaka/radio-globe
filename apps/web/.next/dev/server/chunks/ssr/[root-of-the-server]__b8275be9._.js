...
function pickCountryFeature(station, countries) {
    ...
    if (name) {
        const byName = countries.features.find((feature)=>feature.properties.name.toLowerCase() === name);
        if (byName) return byName;
    }
    const { lat, lng } = station.location;
    return countries.features.find((feature)=>{
        const bounds = getBoundsFromFeature(feature);
        return isWithinBounds(bounds, lat, lng) && containsPoint(feature, lat, lng);
    }) || null;
}
function pickRegionFeature(station, regions, countryFeature) {
    ...
    if (iso3166Candidate) {
        const match = regions.features.find((feature)=>feature.properties.iso3166_2?.toUpperCase() === iso3166Candidate.toUpperCase());
        if (match) return match;
    }
    const { lat, lng } = station.location;
    const countryIso = countryFeature.properties.isoA2?.toUpperCase();
    return regions.features.find((feature)=>{
        const props = feature.properties;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b8275be9._.js.map